...
std::mutex cmd_mutex_;                                   // guards command_interfaces_
std::array<double, N_ARM_JOINTS> command_interfaces_{};  // desired tau, written by controlLoop, read by robotStateLoop
...
private:
  ...
  void robotStateLoop() {
      ...
      // Snapshot the latest commanded torques outside of sim_mutex_ to keep
      // the physics-critical section as short as possible.
      std::array<double, N_ARM_JOINTS> cmd_local;
      {
          std::lock_guard<std::mutex> lock(cmd_mutex_);
          cmd_local = command_interfaces_;
      }
...

      {
          ...
          // "Hardware interface" write: apply the last commanded torques
          // before stepping, same as a real ros2_control write() call.
          for (int i = 0; i < N_ARM_JOINTS; i++) {
              d->ctrl[actuator_ids[i]] = cmd_local[i];
          }
